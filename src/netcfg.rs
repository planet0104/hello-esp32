use std::{sync::{mpsc::sync_channel, Arc}, thread, time::Duration};
use esp_idf_ble::{EspBle, GattServiceEvent, BtUuid, ServiceUuid, GattService, AttributeValue, GattCharacteristic, AutoResponse, GattDescriptor, AdvertiseData};
use esp_idf_hal::interrupt::Mutex;
use esp_idf_svc::nvs::EspDefaultNvs;
use esp_idf_sys::{self as _, ESP_GATT_PERM_READ, ESP_GATT_PERM_WRITE, ESP_GATT_CHAR_PROP_BIT_READ, ESP_GATT_CHAR_PROP_BIT_WRITE, ESP_GATT_UUID_CHAR_CLIENT_CONFIG, esp_gatt_status_t_ESP_GATT_OK, ESP_BLE_ADV_FLAG_GEN_DISC, ESP_BLE_ADV_FLAG_BREDR_NOT_SPT}; // If using the `binstart` feature of `esp-idf-sys`, always keep this module imported
use log::{info, warn};
...
pub fn load_ssid_password() -> Result<NetCfg>{
    let json = nvs::read_string("netcfg")?;
    serde_json::from_str(&json).map_err(|err| anyhow!("{:?}", err) )
}

pub fn save_ssid_password(cfg:&NetCfg) -> Result<()>{
    let json = serde_json::to_string(cfg)?;
    nvs::write_string("netcfg", &json)
}

pub fn clear_ssid_password() -> Result<()>{
    nvs::write_string("netcfg", "")
}

pub fn receive_ssid_password(default_nvs: Arc<EspDefaultNvs>, ble_name:&str) -> Result<NetCfg>{
    
    let mut ble = EspBle::new(ble_name.into(), default_nvs)?;
    ...
    let net_cfg = Arc::new(Mutex::new(None));
    ...
    ble.register_write_handler(char_attr_handle, move |gatts_if, write| {
        if let GattServiceEvent::Write(write) = write {
            if write.is_prep {
                ...
                let msg = String::from_utf8_lossy(&value).to_string();
                info!("received string:{msg}");
                let mut arr = msg.split("|").map(|s| s.to_string());
                let ssid = arr.next();
                let password = arr.next();
                let mut net_cfg = net_cfg_clone.lock();

                if ssid.is_some() && password.is_some(){
                    net_cfg.replace(Some(NetCfg{ ssid: ssid.unwrap(), password: password.unwrap() }));
                }
    ...
    loop {
        ...
        if let Some(cfg) = net_cfg.lock().as_ref(){
        ...
        wait_count += 1;
        if wait_count == 60 * 10{
            return Err(anyhow!("wait timeout!"));
        }
    }
}